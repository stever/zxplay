...
w5100_write_socket_mr( nic_w5100_socket_t *socket, uint8_t b )
{
  ...
  switch( mode ) {
    ...
    case W5100_SOCKET_MODE_TCP:
      ...
      if( flags != 0x20 ) {
        nic_w5100_debug( "w5100: unsupported flags 0x%02x set for TCP mode on socket %d\n", b & 0xf0, socket->id );
        flags = 0x20;
      }
      break;
    case W5100_SOCKET_MODE_UDP:
      ...
      if( flags != 0x00 ) {
        nic_w5100_debug( "w5100: unsupported flags 0x%02x set for UDP mode on socket %d\n", b & 0xf0, socket->id );
        flags = 0;
      }
      break;
    ...
    default:
      nic_w5100_debug( "w5100: unsupported mode 0x%02x set on socket %d\n", b, socket->id );
...
static int
w5100_socket_bind_port( struct nic_w5100_t *self, nic_w5100_socket_t *socket )
{
  ...
  nic_w5100_debug( "w5100: attempting to bind socket %d to %d\n", socket->id, ntohs(sa.sin_port) );
...
static void
w5100_socket_listen( struct nic_w5100_t *self, nic_w5100_socket_t *socket )
{
...
static void
w5100_socket_discon( struct nic_w5100_t *self, nic_w5100_socket_t *socket )
{
...
static void
w5100_socket_close( struct nic_w5100_t *self, nic_w5100_socket_t *socket )
{
  if( socket->fd != compat_socket_invalid ) {
    compat_socket_close( socket->fd );
    socket->fd = compat_socket_invalid;
...
static void
w5100_socket_send( struct nic_w5100_t *self, nic_w5100_socket_t *socket )
{
...
static void
w5100_socket_recv( struct nic_w5100_t *self, nic_w5100_socket_t *socket )
{
  if( socket->state == W5100_SOCKET_STATE_UDP ||
    socket->state == W5100_SOCKET_STATE_ESTABLISHED ) {
...
w5100_write_socket_cr( struct nic_w5100_t *self, nic_w5100_socket_t *socket, uint8_t b )
{
  ...
  switch( b ) {
    ...
    case W5100_SOCKET_COMMAND_RECV:
      ...
      break;
    default:
      nic_w5100_debug( "w5100: unknown command 0x%02x sent to socket %d\n", b, socket->id );
      break;
  }
}
...
uint8_t
nic_w5100_socket_read( struct nic_w5100_t *self, uint16_t reg )
{
...
void
nic_w5100_socket_write( struct nic_w5100_t *self, uint16_t reg, uint8_t b )
{
  nic_w5100_socket_t *socket = &self->socket[(reg >> 8) - 4];
  int socket_reg = reg & 0xff;
...
uint8_t
nic_w5100_socket_read_rx_buffer( struct nic_w5100_t *self, uint16_t reg )
{
...
void
nic_w5100_socket_write_tx_buffer( struct nic_w5100_t *self, uint16_t reg, uint8_t b )
{
...
w5100_socket_process_accept( nic_w5100_socket_t *socket )
{
  ...
  nic_w5100_debug( "w5100: accepted connection from %d on socket %d\n", ntohs(sa.sin_port), socket->id );
